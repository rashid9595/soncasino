...
try {
    // Total menu items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items");
    $stmt->execute();
    $totalItems = $stmt->fetch()['total'];
    
    // Active items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE active = 1");
    $stmt->execute();
    $activeItems = $stmt->fetch()['total'];
    
    // Inactive items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE active = 0");
    $stmt->execute();
    $inactiveItems = $stmt->fetch()['total'];
    
    // Today's updates
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE DATE(updated_at) = CURDATE()");
    $stmt->execute();
    $todayUpdates = $stmt->fetch()['total'];
    
    // Parent menu items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE parent_id IS NULL");
    $stmt->execute();
    $parentItems = $stmt->fetch()['total'];
    
    // Child menu items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE parent_id IS NOT NULL");
    $stmt->execute();
    $childItems = $stmt->fetch()['total'];
    
    // New items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE is_new = 1");
    $stmt->execute();
    $newItems = $stmt->fetch()['total'];
    
    // Popular items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE is_popular = 1");
    $stmt->execute();
    $popularItems = $stmt->fetch()['total'];
    
    // Collapsible items
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM mobile_menu_items WHERE is_collapsible = 1");
    $stmt->execute();
    $collapsibleItems = $stmt->fetch()['total'];
    
    // Average order
    $stmt = $db->prepare("SELECT AVG(order_num) as avg_order FROM mobile_menu_items");
    $stmt->execute();
    $avgOrder = round($stmt->fetch()['avg_order'] ?? 0, 1);
    
} catch (PDOException $e) {
    $error = "İstatistikler yüklenirken bir hata oluştu: " . $e->getMessage();
    $totalItems = $activeItems = $inactiveItems = $todayUpdates = $parentItems = $childItems = $newItems = $popularItems = $collapsibleItems = 0;
    $avgOrder = 0;
}
