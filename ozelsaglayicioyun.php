...
try {
    $stmt = $db->query("SELECT COUNT(*) as totalGames FROM ozelsaglayicioyun");
    $totalGames = $stmt->fetch()['totalGames'];
    
    $stmt = $db->query("SELECT COUNT(*) as activeGames FROM ozelsaglayicioyun WHERE status = 1");
    $activeGames = $stmt->fetch()['activeGames'];
    
    $stmt = $db->query("SELECT COUNT(*) as todayGames FROM ozelsaglayicioyun WHERE DATE(created_at) = CURDATE()");
    $todayGames = $stmt->fetch()['todayGames'];
    
    $stmt = $db->query("SELECT AVG(display_order) as avgOrder FROM ozelsaglayicioyun");
    $avgOrder = round($stmt->fetch()['avgOrder'], 1);
    
    $stmt = $db->query("SELECT COUNT(DISTINCT type) as uniqueTypes FROM ozelsaglayicioyun");
    $uniqueTypes = $stmt->fetch()['uniqueTypes'];
    
    $stmt = $db->query("SELECT COUNT(*) as totalUpdates FROM ozelsaglayicioyun WHERE updated_at IS NOT NULL");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_game'])) {
    try {
        ...
        $stmt = $db->prepare("INSERT INTO ozelsaglayicioyun (name, image_url, redirect_url, alt_text, type, display_order, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
        $stmt->execute([$name, $image_url, $redirect_url, $alt_text, $type, $display_order]);
        
        // Aktivite logu
        $stmt = $db->prepare("INSERT INTO admin_activity_logs (admin_id, action, details, ip_address, created_at) VALUES (?, ?, ?, ?, NOW())");
        $stmt->execute([$_SESSION['admin_id'], 'add_provider_game', "Yeni özel sağlayıcı oyunu eklendi: $name", $_SERVER['REMOTE_ADDR']]);
    ...
    header("Location: ozelsaglayicioyun.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_game'])) {
    try {
        ...
        $stmt = $db->prepare("UPDATE ozelsaglayicioyun SET name = ?, image_url = ?, redirect_url = ?, alt_text = ?, type = ?, display_order = ?, updated_at = NOW() WHERE id = ?");
        $stmt->execute([$name, $image_url, $redirect_url, $alt_text, $type, $display_order, $game_id]);
        
        // Aktivite logu
        $stmt = $db->prepare("INSERT INTO admin_activity_logs (admin_id, action, details, ip_address, created_at) VALUES (?, ?, ?, ?, NOW())");
        $stmt->execute([$_SESSION['admin_id'], 'edit_provider_game', "Özel sağlayıcı oyunu güncellendi: ID $game_id", $_SERVER['REMOTE_ADDR']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_game'])) {
    try {
        ...
        $stmt = $db->prepare("DELETE FROM ozelsaglayicioyun WHERE id = ?");
        $stmt->execute([$game_id]);
        
        // Aktivite logu
        $stmt = $db->prepare("INSERT INTO admin_activity_logs (admin_id, action, details, ip_address, created_at) VALUES (?, ?, ?, ?, NOW())");
        $stmt->execute([$_SESSION['admin_id'], 'delete_provider_game', "Özel sağlayıcı oyunu silindi: ID $game_id", $_SERVER['REMOTE_ADDR']]);
...
// Tüm oyunları getir
$stmt = $db->query("SELECT * FROM ozelsaglayicioyun ORDER BY display_order, id");
$games = $stmt->fetchAll();
...
.provider-games-card .card-header {
    background: linear-gradient(135deg, var(--primary-color) 0%, #0056b3 100%);
    color: white;
    padding: 1.5rem;
    border-bottom: none;
...
.provider-games-card .card-header h5 {
    margin: 0;
    font-weight: 600;
    font-size: 1.25rem;
}

.provider-games-card .card-body {
    padding: 0;
}
...
<div class="dashboard-header">
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col">
                <h1><i class="bi bi-controller me-3"></i>Özel Sağlayıcı Oyunları</h1>
                <p>Özel sağlayıcı oyunlarını yönetin ve düzenleyin</p>
            </div>
            <div class="col-auto">
    ...
    <div class="provider-games-card">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="providerGamesTable" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                            <th>Sıra</th>
                            <th>İsim</th>
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <p>İlk özel sağlayıcı oyununu eklemek için yukarıdaki butonu kullanın.</p>
                        ...
                        <?php else: ?>
                            <?php foreach ($games as $game): ?>
                                <tr>
                                    <td>
                                        <span class="badge bg-primary"><?php echo $game['display_order']; ?></span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <img src="<?php echo htmlspecialchars($game['image_url']); ?>" 
                                             alt="<?php echo htmlspecialchars($game['alt_text']); ?>" 
                                             class="game-image"
                                    ...
                                    <td>
                                        <div class="text-truncate" style="max-width: 200px;" title="<?php echo htmlspecialchars($game['redirect_url']); ?>">
                                            <?php echo htmlspecialchars($game['redirect_url']); ?>
                                        </div>
                                    </td>
                                    <td>
                                        <small class="text-muted"><?php echo htmlspecialchars($game['alt_text']); ?></small>
                                    </td>
                                    <td>
                                        <span class="badge bg-info"><?php echo htmlspecialchars($game['type']); ?></span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <button type="button" class="btn btn-primary btn-sm edit-game-btn" 
                                                    ...
                                                    data-redirect-url="<?php echo htmlspecialchars($game['redirect_url']); ?>"
                                                    data-alt-text="<?php echo htmlspecialchars($game['alt_text']); ?>"
                                                    data-type="<?php echo htmlspecialchars($game['type']); ?>"
                                                    data-display-order="<?php echo $game['display_order']; ?>">
            ...
            <form method="POST" id="addGameForm">
                <div class="modal-body">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="new_name" class="form-label">
                                    <i class="bi bi-tag me-2"></i>Oyun İsmi
                                </label>
                                <input type="text" class="form-control" id="new_name" name="name" 
                                       placeholder="Oyun ismi" required>
                                <div class="form-text">Oyunun görünen ismini girin</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="new_type" class="form-label">
                                    <i class="bi bi-collection me-2"></i>Tip
                                </label>
                                <input type="text" class="form-control" id="new_type" name="type" 
                                       placeholder="slot, casino, poker" required>
                                <div class="form-text">Oyun tipini belirtin</div>
                            </div>
                        </div>
                    </div>
                    
                    <div class="mb-3">
                        <label for="new_image_url" class="form-label">
                            <i class="bi bi-image me-2"></i>Resim URL
                        </label>
                        <input type="url" class="form-control" id="new_image_url" name="image_url" 
                               placeholder="https://example.com/image.jpg" required>
                        <div class="form-text">Oyun resminin URL adresini girin</div>
                    </div>
                    
                    <div class="mb-3">
                        <label for="new_redirect_url" class="form-label">
                            <i class="bi bi-link-45deg me-2"></i>Yönlendirme URL
                        </label>
                        <input type="url" class="form-control" id="new_redirect_url" name="redirect_url" 
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="mb-3">
                                <label for="new_alt_text" class="form-label">
                                    <i class="bi bi-text-paragraph me-2"></i>Alt Metin
                                </label>
                                <input type="text" class="form-control" id="new_alt_text" name="alt_text" 
                                       placeholder="Oyun açıklaması">
                                <div class="form-text">Resim için alt metin açıklaması</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label for="new_display_order" class="form-label">
                                    <i class="bi bi-sort-numeric-up me-2"></i>Sıra
                                </label>
                                <input type="number" class="form-control" id="new_display_order" name="display_order" 
                                       placeholder="1" min="1" required>
                                <div class="form-text">Görüntülenme sırası</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <form method="POST" id="editGameForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control" id="edit_redirect_url" name="redirect_url" required>
                        <div class="form-text">Oyun linkinin URL adresini girin</div>
                    </div>
...
$(document).ready(function() {
    ...
    $('.edit-game-btn').on('click', function() {
        ...
        $('#edit_redirect_url').val(redirectUrl);
    ...
    $('#addGameForm, #editGameForm').on('submit', function(e) {
        ...
        const redirectUrl = $(this).find('input[name="redirect_url"]').val();
        const type = $(this).find('input[name="type"]').val();
        const displayOrder = $(this).find('input[name="display_order"]').val();

        if (!name || !imageUrl || !redirectUrl || !type || !displayOrder) {
            e.preventDefault();
            Swal.fire({
                icon: 'error',
                title: 'Hata!',
        ...
        if (!isValidUrl(imageUrl) || !isValidUrl(redirectUrl)) {
            e.preventDefault();
            Swal.fire({
                icon: 'error',
                title: 'Hata!',
...
$pageTitle = "Özel Sağlayıcı Oyunları";
