...
try {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM xpayayarlarr");
    $stmt->execute();
    $totalSettings = $stmt->fetch()['total'];
    
    // Check if API URL is configured
    $stmt = $db->prepare("SELECT COUNT(*) as configured FROM xpayayarlarr WHERE apiurl IS NOT NULL AND apiurl != ''");
    $stmt->execute();
    $apiUrlConfigured = $stmt->fetch()['configured'];
    
    // Check if Site Key is configured
    $stmt = $db->prepare("SELECT COUNT(*) as configured FROM xpayayarlarr WHERE site_key IS NOT NULL AND site_key != ''");
    $stmt->execute();
    $siteKeyConfigured = $stmt->fetch()['configured'];
    
    // Check if settings are complete
    $stmt = $db->prepare("SELECT COUNT(*) as complete FROM xpayayarlarr WHERE apiurl IS NOT NULL AND apiurl != '' AND site_key IS NOT NULL AND site_key != ''");
    $stmt->execute();
    $settingsComplete = $stmt->fetch()['complete'];
    
    // Get last update time
    $stmt = $db->prepare("SELECT MAX(updated_at) as last_update FROM xpayayarlarr");
    $stmt->execute();
    $lastUpdate = $stmt->fetch()['last_update'];
    
    // Count recent updates (last 7 days)
    $stmt = $db->prepare("SELECT COUNT(*) as recent FROM xpayayarlarr WHERE updated_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)");
    $stmt->execute();
    $recentUpdates = $stmt->fetch()['recent'];
    
} catch (PDOException $e) {
    $error = "İstatistikler yüklenirken bir hata oluştu: " . $e->getMessage();
    $totalSettings = $apiUrlConfigured = $siteKeyConfigured = $settingsComplete = $recentUpdates = 0;
    $lastUpdate = null;
}
...
try {
    $db->exec("
        CREATE TABLE IF NOT EXISTS `xpayayarlarr` (
    ...
    $stmt = $db->query("SELECT COUNT(*) FROM xpayayarlarr");
    $count = $stmt->fetchColumn();
    
    if ($count == 0) {
        $db->exec("
            INSERT INTO `xpayayarlarr` (`id`, `apiurl`, `site_key`) VALUES
            (1, 'https://testapi.xpayio.com/api/services/deposit', 'c7a8b0aff381d18a98b35e57a17d5a93')
        ");
    }
} catch (PDOException $e) {
    // Log the error
    error_log('XPay Settings Table Error: ' . $e->getMessage());
}
...
// Get settings
$stmt = $db->prepare("SELECT * FROM xpayayarlarr WHERE id = 1");
$stmt->execute();
...
<div class="dashboard-header animate-fade-in">
    ...
    <p class="dashboard-subtitle">XPay API entegrasyon ayarlarını yönetin ve yapılandırın.</p>
</div>
...
<div class="stat-grid animate-fade-in" style="animation-delay: 0.1s">
    ...
    <div class="stat-card api">
        ...
        <div class="stat-number"><?php echo number_format($apiUrlConfigured); ?></div>
        <div class="stat-title">API URL Yapılandırıldı</div>
    </div>
    ...
    <div class="stat-card key">
        ...
        <div class="stat-number"><?php echo number_format($siteKeyConfigured); ?></div>
        <div class="stat-title">Site Key Yapılandırıldı</div>
    </div>
    ...
    <div class="stat-card updates">
        ...
        <div class="stat-number"><?php echo number_format($recentUpdates); ?></div>
        <div class="stat-title">Son 7 Gün Güncelleme</div>
    </div>
</div>

<div class="settings-card animate-fade-in" style="animation-delay: 0.2s">
    <div class="settings-header">
        <h6><i class="bi bi-gear-fill"></i>XPay API Entegrasyon Ayarları</h6>
    ...
    <div class="settings-body">
        <div class="setting-item">
            ...
            <div class="setting-value">
                <input type="text" 
                       class="form-control" 
                       value="<?php echo htmlspecialchars($settings['apiurl']); ?>" 
                       id="apiurl"
                       placeholder="https://api.xpayio.com/api/services/deposit"
                       <?php if (!$isAdmin): ?>readonly<?php endif; ?>>
                <?php if ($isAdmin): ?>
                <button class="btn btn-primary" onclick="updateSetting('apiurl')">
            ...
            <div class="alert alert-info">
                ...
                XPay API'ye erişim için kullanılacak URL adresi. Bu URL, ödeme işlemleri için gerekli endpoint'i içermelidir.
            </div>
        </div>
        ...
        <div class="setting-item">
            ...
            <div class="setting-value">
                <input type="text" 
                       class="form-control" 
                       value="<?php echo htmlspecialchars($settings['site_key']); ?>" 
                       id="site_key"
                       placeholder="XPay site anahtarınızı buraya girin"
                       <?php if (!$isAdmin): ?>readonly<?php endif; ?>>
                <?php if ($isAdmin): ?>
                <button class="btn btn-primary" onclick="updateSetting('site_key')">
                    <i class="bi bi-save me-1"></i>Kaydet
                </button>
                <?php endif; ?>
            </div>
            <div class="alert alert-info">
                <i class="bi bi-info-circle me-2"></i>
                XPay API isteklerini yetkilendirmek için kullanılan site anahtarı. Bu anahtar, güvenli ödeme işlemleri için gereklidir.
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const siteKeyInput = document.getElementById('site_key');
    ...
    if (apiUrlInput) {
        apiUrlInput.addEventListener('blur', function() {
            ...
            if (value && !value.startsWith('http')) {
                ...
                this.title = 'Geçerli bir URL giriniz (http:// veya https:// ile başlamalı)';
            } else {
    ...
    if (siteKeyInput) {
        siteKeyInput.addEventListener('blur', function() {
            ...
            if (value && value.length < 10) {
                this.style.borderColor = 'var(--error-red)';
                this.title = 'Site anahtarı en az 10 karakter olmalıdır';
            } else {
                this.style.borderColor = '';
                this.title = '';
            }
